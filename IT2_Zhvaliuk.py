...
def create_profile(connection, data_path):
    ...
    # открываем каждый файл и парсим данные
    profile_data = os.path.join(data_path, "profile")
    # создаем insert query template
    insert_query = """INSERT INTO profile VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
    # создание цикла для обработки файлов
...
#Расчет просрочки
def compute_delay(connection):
    connection.create_table(type_="payment_delay")
    select_ids = """SELECT DISTINCT(contract_id) FROM profile_contract_payment"""
    select_client = """SELECT id, issue_date, payment_date FROM profile_contract_payment WHERE contract_id = '{}'"""
    insert_values = """INSERT INTO payment_delay VALUES (?, ?, ?, ?, ?)"""
    contract_ids = connection.cursor.execute(select_ids).fetchall()
    contract_ids = [item[0] for item in contract_ids]
    param_set = lambda x: 1 if x > 90 else 0
    for contract_id in contract_ids:
        connection.cursor.execute(select_client.format(contract_id))
        data = connection.cursor.fetchall()
        data = [(item[0], datetime.strptime(item[1], "%m.%d.%Y"), datetime.strptime(item[2], "%m.%d.%Y")) for item in
                data]
        frame = pd.DataFrame(data, columns=["id", "issue_date", "payment_date"])
        frame.sort_values(by="payment_date", inplace=True)
        values = []
        for i, j in zip(frame["payment_date"], frame["payment_date"][1::]):
            delta = j - i
            values.append([contract_id, i.strftime("%m.%d.%Y"), j.strftime("%m.%d.%Y"), delta.days,
                           param_set(delta.days)])
        for item in values:
            connection.cursor.execute(insert_values, item)
        connection.connection.commit()
...
def search_engine(data):
    search = input("Enter parameter for filtering resulting values (Format 'column_name'='value' _>")
    request = search.split("=")
    value, type_ = check_input(request[1])
    request = [request[0], value]
    # устанавливаем фильтр дефолтера
    if type_ == "str" or type_ == "int":
        good_data = data[(data[request[0]] == request[1]) & (data["status"] == "Non-Default")].copy()
        bad_data = data[(data[request[0]] == request[1]) & (data["status"] == "Default")].copy()
    elif type_ == float:
        good_data = data[(data[request[0]] >= request[1]) & (data["status"] == "Non-Default")].copy()
        bad_data = data[(data[request[0]] >= request[1]) & (data["status"] == "Default")].copy()
    else:
        return 0, 0, 0
    return good_data, bad_data, request[0]
...
def smart_drow(bad_data, good_data, key):
    ...
    # setup switch
    if key == "gender":
        # сюда можно добавлять и другие бинарные переменные
        bad_value = bad_data[key].count()
        good_value = good_data[key].count()
        print("WOE=%s" % str(WOE(good_value, bad_value)))
        print("IV=%s" % str(IV(good_value, bad_value)))
    if key == "amount_due":
        bad_value = bad_data[key].count()
        good_value = good_data[key].count()
...
def interface(connection):
    ...
    while not correct:
        ...
        try:
            ...
            print("You entered >%s<. Make sure that it's equal to required format" % horizon)
    # ищем клиентов с просрочкой более 90 + и объединим в большую таблицу
    ...
    data = []
    # в будущем строки ниже облегчат нам жизнь
    ...
    # я разбиваю всех дефолтеров на группы по N дней, потом считаю количесво записей в каждой из групп и беру среднее
    # округляя вверх
